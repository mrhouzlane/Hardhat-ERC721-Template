...
describe("Doodles", function() {
  ...
  describe('setIsAllowListActive', function(){
    ...
    it('setIsAllowListActive should be set to true by the owner', async function () {
        ...
        await doodlesContract.connect(owner).setIsAllowListActive(_isAllowListActive); 
        expect(doodlesContract.connect(owner).setIsAllowListActive(_isAllowListActive), _isAllowListActive)
    ...
    describe('numAvailableToMint', function(){
      it("should return number Available of Tokens to Mint", async function() {
        ...
        await doodlesContract.connect(owner).setIsAllowListActive(true);
        await doodlesContract.connect(owner).setAllowList([addr1.address, addr2.address], 10)

        expect(await doodlesContract.numAvailableToMint(addr1.address)).to.be.equal(10);
        expect(await doodlesContract.numAvailableToMint(addr2.address)).to.be.equal(10);
    ...
    describe('mintAllowList', function(){
      ...
      it("Should be reverted because the caller exceeds max token", async function() {
        ...
        for (let i=0; i <50; i++){
          await doodlesContract.connect(owner).setAllowList([addrs[i].address], 200)
          await doodlesContract.connect(addrs[i]).mintAllowList(200, overrides)
        }

        await doodlesContract.connect(owner).setAllowList([addrs[50].address], 200)
        await expect(doodlesContract.connect(addrs[50]).mintAllowList(1, overrides)).to.be.revertedWith(
          "Purchase would exceed max tokens"
        )

        // Testing for the MAX_SUPPLY if it works 
        await doodlesContract.connect(owner).setAllowList([addrs[49].address], 200)
        expect(await doodlesContract.ownerOf(9999)).to.be.equal(addrs[49].address)
    ...
    describe('setBaseURI', function(){
      it("should set the BaseURI by the owner", async function() {
        ...
        const baseurl = "ipfs://test.url/"
        await expect (doodlesContract.connect(addr1).setBaseURI(baseurl)).to.be.revertedWith(
    ...
    describe('setProvenance', function() {
      it("should setProvenance by the owner", async function() {
        ...
        await expect (doodlesContract.connect(addr1).setProvenance("from Paris")).to.be.revertedWith(
          'Ownable: caller is not the owner'
        );
      });
    ...
    describe('reserve', function() {
      it("should mint the reserve by the Owner", async function() {
        await expect (doodlesContract.connect(addr1).reserve(200)).to.be.revertedWith(
          'Ownable: caller is not the owner'
        );
      })
    ...
    describe('setSaleState', function() {
      it("should setSaleState by the Owner", async function() {
        await expect (doodlesContract.connect(addr1).setSaleState(true)).to.be.revertedWith(
          'Ownable: caller is not the owner'
        );
      })
    ...
    describe('mint', function() {
      it("should revert if saleIsActive is false", async function() {
        ...
        await expect(doodlesContract.connect(addr1).mint(1, overrides)).to.be.revertedWith(
    ...
    describe('withdraw', function() {
      it("should revert if other address than Owner", async function() {
        await expect (doodlesContract.connect(addr1).withdraw()).to.be.revertedWith(
          "Ownable: caller is not the owner")
      });
